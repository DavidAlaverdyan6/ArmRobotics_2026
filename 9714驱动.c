...
#define LENS_I2C_BUSNUM 0////1 liyunpen modified
...
static int s4FM50AF_ReadReg(unsigned short * a_pu2Result)
{
    ...
    if (i4RetValue < 0) 
    {
		...
		//*a_pu2Result = (((u16)(pBuff[0] & 0x03)) << 8) + pBuff[1];///liyunpen
...
static int s4FM50AF_WriteReg(u16 a_u2Data)
{
    int  i4RetValue = 0;
		...
		//char puSendCmd[2] = {(char)(((a_u2Data >> 8) & 0x03) | 0x00), (char)(a_u2Data & 0x3f)};
char puSendCmd[2] = {(char)(a_u2Data >> 4) , (char)((a_u2Data & 0xF) << 4)};////liyunpen  proved by Eric 150903
